    ...
    http
        ...
        .csrf(csrf -> csrf.csrfTokenRepository(csrfCookies)
            .requireCsrfProtectionMatcher(request -> CsrfFilter.DEFAULT_CSRF_MATCHER.matches(request)
                && request.getRequestURI().substring(request.getContextPath().length()).startsWith("/api/v1/auth/")))
        .authorizeHttpRequests(auth -> auth
            .dispatcherTypeMatchers(DispatcherType.ERROR).permitAll()
            .requestMatchers("/api/v1/auth/forgot-password", "/api/v1/auth/verify-reset-otp",
                "/api/v1/auth/reset-password").denyAll()
            .requestMatchers(HttpMethod.GET, "/api/v1/auth/csrf").permitAll()
            .requestMatchers(HttpMethod.POST, "/api/v1/auth/login", "/api/v1/auth/register",
                "/api/v1/auth/verify-user", "/api/v1/auth/refresh-token", "/api/v1/auth/logout").permitAll()
            .requestMatchers("/api/v1/auth/myProfile", "/api/v1/auth/updateProfile").authenticated()
            // User has no persisted admin role yet. Do not manufacture administrative authority.
        ...
        .oauth2ResourceServer(resource -> resource
            // Cookie-auth endpoints must not inherit the resource server's Bearer CSRF exemption.
            // Ignoring stale access headers here also lets refresh/logout work after access expiry.
