...
export function useAgentAudioVisualizerWave({
  ...
  useEffect(() => {
    if (state === 'speaking') {
      animateAmplitude(0.015 + 0.4 * resolvedVolume, { duration: 0 });
      animateFrequency(20 + 60 * resolvedVolume, { duration: 0 });
    }
  }, [state, resolvedVolume, animateAmplitude, animateFrequency]);
